...
import { SedesModuleNotAvailableError, createAdminSede, getAdminSedes } from "@/lib/admin"
...
export async function GET(request: Request) {
  try {
    ...
    const sedes = await getAdminSedes(tenantSchema)
    return NextResponse.json({ ok: true, sedes }, { status: 200 })
  } catch (error) {
    ...
    console.error("Admin sedes API error", error)
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return jsonError(400, {
      ...
      error: "Datos inválidos para crear la sede.",
    })
  }
  ...
  try {
    ...
    const sede = await createAdminSede({
    ...
    return NextResponse.json({ ok: true, sede }, { status: 201 })
  } catch (error) {
    if (error instanceof SedesModuleNotAvailableError) {
      return jsonError(409, {
        code: "SEDES_MODULE_NOT_AVAILABLE",
    ...
    console.error("Admin create sede API error", error)
    return jsonError(500, {
      code: "SERVER_ERROR",
