...
export default function AdminAgendamientosPage() {
  ...
  const loadCatalogs = useCallback(async () => {
    try {
      ...
      const [employeesResponse, clientsResponse, servicesResponse] = await Promise.all([
        fetch("/api/admin/employees", { cache: "no-store", headers }),
        fetch("/api/admin/clients", { cache: "no-store", headers }),
        fetch("/api/admin/services", { cache: "no-store", headers }),
      ])

      const employeesData: EmployeesResponse = await employeesResponse.json().catch(() => ({}))
      const clientsData: ClientsResponse = await clientsResponse.json().catch(() => ({}))
      const servicesData: ServicesResponse = await servicesResponse.json().catch(() => ({}))

      if (employeesResponse.ok) {
        const list = Array.isArray(employeesData.employees) ? employeesData.employees : []
        setEmployeesCatalog([...list].sort((a, b) => a.name.localeCompare(b.name, "es", { sensitivity: "base" })))
      }

      if (clientsResponse.ok) {
        const list = Array.isArray(clientsData.clients) ? clientsData.clients : []
        setClientsCatalog([...list].sort((a, b) => a.name.localeCompare(b.name, "es", { sensitivity: "base" })))
      }

      if (servicesResponse.ok) {
        const list = Array.isArray(servicesData.services) ? servicesData.services : []
        setServicesCatalog(
          [...list]
    ...
    try {
      ...
      if (quickActionMode === "create") {
        ...
        payload = {
          ...
          employeeId: quickActionSlot.employeeId,
          ...
          <>
            ...
            <Card className="border-border/60 bg-gradient-to-b from-background/80 via-background to-background/95">
              <CardHeader className="space-y-2">
                ...
                <CardDescription>
                  Visualiza qué empleado trabaja, en qué horario y qué servicio realiza por día, semana, mes o en lista.
                </CardDescription>
              </CardHeader>
