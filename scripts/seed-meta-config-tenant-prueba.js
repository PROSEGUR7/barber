  ...
  try {
    const result = await pool.query(
      ...
      [
        tenantSchema,
        tenantId,
        process.env.META_ACCOUNT_NAME?.trim() || null,
        required("META_ACCESS_TOKEN"),
        required("META_VERIFY_TOKEN"),
        required("META_PHONE_NUMBER_ID"),
        process.env.META_DISPLAY_PHONE_NUMBER?.trim() || null,
        process.env.META_BUSINESS_ACCOUNT_ID?.trim() || null,
        process.env.META_GRAPH_VERSION?.trim() || "v22.0",
        process.env.META_WEBHOOK_SECRET?.trim() || null,
        process.env.N8N_WEBHOOK_URL?.trim() || null,
        process.env.N8N_API_KEY?.trim() || null,
      ],
    )

    console.log("Configuración Meta en tenant_prueba insertada/actualizada correctamente:")
...
main().catch((error) => {
  console.error("Error al insertar configuración en tenant_prueba")
