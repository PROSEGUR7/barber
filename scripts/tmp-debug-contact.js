...
(async () => {
  ...
  const r = await pool.query(`
    SELECT id, sent_at::text, message_type, message_text,
           ...
           raw_payload->'message'->'contacts' AS raw_contacts,
           raw_payload->'message'->'contact' AS raw_contact,
  ...
  const target = r.rows.find((x) => x.raw_type === 'contacts' || x.raw_contact || x.raw_contacts || x.message_type === 'contacts');
  if (target) {
